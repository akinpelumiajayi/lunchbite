...
> Generated: 2026-08-20 21:15  |  Model: groq/qwen/qwen3.6-27b  |  Cases: 18  |  Pipelines: no_llm, neural_rag, neurosymbolic, no_rag
...
> Results are confined to the 29-recipe corpus and 18-case benchmark described herein.

---

## 1. Executive Summary

Four RAG pipelines were benchmarked against a fixed 18-case test suite, of
...
|----------|--------------------------|----------|---------------------|-------------------|
| No-LLM baseline | 0.0% (0/18 cases) | 100.0% | 100.0% | 0.0% (0/2 cases) |
| Neural-only RAG | 0.0% (0/17 cases) | 100.0% | 100.0% | 0.0% (0/2 cases) |
| Neuro-symbolic RAG | 0.0% (0/17 cases) | 100.0% | 100.0% | 0.0% (0/2 cases) |
| No-RAG control | 62.5% (10/16 cases) | 94.1% | 35.3% | 50.0% (1/2 cases) |
...
> **⚠️ 1 of 18 case-runs failed and are excluded from the rates below.** A failed run (rate limit, refused API key, unparseable output) produces no menus, which is indistinguishable from a safe refusal unless it is excluded — so the figures here describe only the runs that completed.
>
> | Pipeline | Case-runs scored | Failed and excluded |
> |---|---|---|
> | No-LLM baseline | 18 | 0 |
> | Neural-only RAG | 17 | 1 |
> | Neuro-symbolic RAG | 17 | 1 |
> | No-RAG control | 17 | 1 |
>
...
|--------|---|---|---|---|
| Allergen violation rate | 0.0% | 0.0% | 0.0% | 62.5% |
| Adversarial bypass rate | 0.0% | 0.0% | 0.0% | 50.0% |
| Hallucinated recipe ID rate | 0.0% | 0.0% | 0.0% | 2.1% |
| Cases with final menus | 18 | 17 | 17 | 16 |

| Symbolic gate metric | No-LLM baseline | Neural-only RAG | Neuro-symbolic RAG | No-RAG control |
|----------------------|---|---|---|---|
| Pre-filter precision | 0.949 | — | 0.959 | — |
...
|---|---|---|---|---|---|
| `neurosymbolic` vs `neural_rag` | 17 | 0 vs 0 | 0/0 | 1.0000 | no |
| `neurosymbolic` vs `no_rag` | 17 | 0 vs 10 | 10/0 | 0.0020 | **yes** |
| `no_llm` vs `neural_rag` | 17 | 0 vs 0 | 0/0 | 1.0000 | no |
...
| Metric | No-LLM baseline | Neural-only RAG | Neuro-symbolic RAG | No-RAG control |
|--------|---|---|---|---|
| Relevance 1–5 | 4.111 [3.667, 4.500] (n=18) | 4.765 [4.529, 4.941] (n=17) | 4.647 [4.412, 4.882] (n=17) | 4.375 [3.688, 4.938] (n=16) |
| Faithfulness 0–1 | 0.884 [0.800, 0.957] (n=18) | 0.994 [0.982, 1.000] (n=17) | 0.954 [0.882, 1.000] (n=17) | 0.112 [0.027, 0.219] (n=16) |
| Naturalness 1–5 | 3.778 [3.556, 3.944] (n=18) | 4.941 [4.824, 5.000] (n=17) | 4.765 [4.471, 5.000] (n=17) | 4.375 [4.125, 4.625] (n=16) |
...
|--------|-----------------|--------|-------|---------|
| Relevance | -0.118 | [-0.294, 0.000] | 17 | no difference detected |
| Faithfulness | -0.041 | [-0.116, 0.012] | 17 | no difference detected |
| Naturalness | -0.176 | [-0.353, 0.000] | 17 | no difference detected |
...
The 2 cases carrying a prompt injection (of 4 in the adversarial category) include injections such as:
...
**Its output is structured, but it is not unreadable, and the judge does not score it as such.** Naturalness came out at **3.778/5** (n=18), against 4.941 for `neural_rag`. The template emits grammatical English — *"Meets all allergen and nutrition constraints for age 7"* — so a rubric asking whether a recommendation reads naturally finds something to reward.
...
# Full run — all 4 pipelines, 18 cases, live LLM:
python3 run_all.py
...
Results file used for this report: `run_20260820_205415.json`
Eval file used: `run_20260820_205415_eval.json`